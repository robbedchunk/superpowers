...
description: Update and reinstall the Superpowers plugin from the robbedchunk/superpowers GitHub fork in ~/.codex, ~/.codex-a, ~/.codex-b, and Claude Code (~/.claude). Use when the user says "update superpowers", "update the superpowers", "update superpowers plugin", "refresh superpowers", or asks to pull their latest Superpowers changes.
---
...
`superpowers@superpowers-dev` in all three Codex homes and in Claude Code (user
...
four homes through `scripts/install-user-skills.sh` from the refreshed
Claude marketplace checkout. These standalone skills remain outside the
Superpowers plugin's opt-in skill directory.

After it finishes:

1. Verify all four homes report the plugin as installed and enabled.
2. Verify the two standalone skills match the canonical repository copies in
   all four homes, and both delegation workflows default to
