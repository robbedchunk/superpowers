...
- **Declared-trigger opt-in:** A skill whose own opt-in boundary declares additional triggers may be invoked under those declared triggers; treat them as explicit opt-in for that skill only. Three skills declare them: `superpowers:plan-delegate-review`, which activates when the user enters plan mode for implementation work; `superpowers:grilling`, which activates when the user asks to be grilled ("grill me", "grill this", "grill the plan"); and `superpowers:full-stack-split`, which activates when the user asks for the full-stack split ("full-stack split", "split workflow", "split this front and back").
...
- "Grill me on this migration plan" matches `superpowers:grilling`'s declared trigger; ask before transitioning to another workflow.
